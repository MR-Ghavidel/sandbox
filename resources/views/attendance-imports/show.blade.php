...
@php
    $statusLabels = [
        AttendanceImporter::STATUS_NEW => ['جدید', 'bg-sky-100 text-sky-700'],
        AttendanceImporter::STATUS_CHANGED => ['تغییر', 'bg-amber-100 text-amber-800'],
        AttendanceImporter::STATUS_UNCHANGED => ['بدون تغییر', 'bg-slate-100 text-slate-600'],
        AttendanceImporter::STATUS_EMPTY => ['بدون داده', 'bg-slate-100 text-slate-400'],
    ];
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @foreach ($rows as $row)
                    ...
                    <tr @class(['text-slate-400' => $row['status'] === AttendanceImporter::STATUS_EMPTY])>
                        <td class="px-4 py-2 whitespace-nowrap">
                            <span class="font-medium">{{ JalaliDate::format($row['date'], 'EEEE') }}</span>
                            <span class="text-xs text-slate-500">{{ JalaliDate::format($row['date'], 'd MMMM') }}</span>
                        </td>
                        <td class="px-4 py-2"><span class="rounded-full px-2 py-0.5 text-xs {{ $statusClasses }}">{{ $statusLabel }}</span></td>
                        <td class="px-4 py-2 text-slate-500" dir="ltr">{{ $row['current'] ? $pairsText($row['current']->pairs) : '' }}</td>
                        <td class="px-4 py-2">
                            <span dir="ltr">{{ $pairsText($row['merged']->pairs) }}</span>
                            @if (! $row['merged']->isWorkDay)
                                <span class="ms-2 rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600">{{ $row['merged']->note ?? 'غیرکاری' }}</span>
                            @endif
                            @if ($row['merged']->hasIncompletePair())
                                <span class="ms-2 rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">ناقص</span>
                            @endif
                        </td>
